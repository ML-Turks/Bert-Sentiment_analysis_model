...
def _get_train_data_loader(batch_size, training_dir, is_distributed):
    ...
    dataset = pd.read_csv(os.path.join(training_dir, "train.csv"),header=None,usecols=[0, 1],names=["label", "sentence"])
...
def _get_test_data_loader(test_batch_size, test_dir):
    dataset = pd.read_csv(os.path.join(test_dir, "test.csv"),header=None,usecols=[0, 1],names=["label", "sentence"])
    sentences = dataset.sentence.values
    labels = dataset.label.values
